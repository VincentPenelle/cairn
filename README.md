...
Warning: Cairn only accepts grammars whose starting symbol is named "main" (for technical reasons and keeping the functor a bit lighter).
...
- Cairn only currently accepts grammars whose starting symbol is called "main" (mainly as it is an usual convention, and for being able to simply the module generated by menhir as is to the `Make` functor).
